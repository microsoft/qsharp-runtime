...
using static NewTracer.GateDecomposition.DecompositionUtils;

namespace NewTracer.MetricCollection
{
    /// This class is for the most part UNTESTED and thus should not currently be relied upon
    public class DefaultDecomposer : DecomposerBase
    {
        ...
        public DefaultDecomposer(IQuantumProcessor targetProcessor) : base(targetProcessor)
        {
        }
        ...
        {
            this.ControlledRFrac(ToIQArray(), axis, numerator, power, qubit);
        }
        ...
        {
            this.ControlledR1(ToIQArray(), theta, qubit);
        }
        ...
        {
            this.ControlledR1Frac(ToIQArray(), numerator, power, qubit);
        }
        ...
        public override void ControlledR1Frac(IQArray<Qubit> controls, long numerator, long power, Qubit qubit)
        {
            if (power >= 0)
            {
                ...
                {
                    double numeratorD = Math.PI * kModPositive;
                    double phi = numeratorD * Math.Pow(2.0, -n);
        ...
        {
            this.ControlledExp(ToIQArray(), paulis, theta, qubits);
        }
        ...
        {
            this.ControlledExpFrac(ToIQArray(), paulis, numerator, power, qubits);
        }
        ...
        protected virtual void ApplyWithOneLessControl(ControlledOp op, IQArray<Qubit> controls, Qubit qubit)
        {
            ...
            Qubit helper = AllocateAncillaQubit();

            this.ControlledX(ToIQArray(control0, control1), helper);

            //not the real qubit array: the first should be the ancilla but this is a hack for mocking
            IQArray<Qubit> oneLessControl = controls.Slice(new QRange(1, controls.Length - 1));
            Qubit target = controls.Length > 1 ? qubit : helper;

            op(oneLessControl, target);

            this.H(helper);
            this.M(helper); //TODO: how to implement code below instead of this line?
        ...
        protected virtual IQArray<Qubit> ToIQArray(params Qubit[] qubits)
        {
            ...
            return new QArray<Qubit>(qubits);
        }
