...
namespace Microsoft.Quantum
{
    ...
    [<Fact>]
    let ``buildDeclarationAttributes - emptyElements`` () =    
        let testHeaders fileName (expected: string list) =
            let expected = expected |> List.map (fun e -> e.Replace("%%%", (Uri(Path.GetFullPath fileName)).AbsolutePath))
            let tree = parse [fileName]
            let ns   = tree |> Seq.head
            let actual = buildDeclarationAttributes (ns.Elements |> Seq.toList) |> List.map formatSyntaxTree
            List.zip (expected |> List.map clearFormatting |> List.sort) (actual |> List.map clearFormatting |> List.sort) 
            |> List.iter Assert.Equal

        let asAttribute attrName (serialization : string) = sprintf "[assembly: %s(\"%s\")]" attrName (serialization.Replace("\"", "\\\""))
        let qualifiedName ns name = {Namespace = ns |> NonNullable<string>.New; Name = name |> NonNullable<string>.New}
        let udtName ns name r = 
            let range = r |> function 
                | Some (rstart, rend) -> ({Line = fst rstart; Column = snd rstart}, {Line = fst rend; Column = snd rend}) |> Value
                | None -> Null
            {Namespace = ns |> NonNullable<string>.New; Name = name |> NonNullable<string>.New; Range = range}
        let intrinsic = CallableInformation.New(ResolvedCharacteristics.Empty, {InferredCallableInformation.NoInformation with IsIntrinsic = true})

        let simpleSignature argType rType = {
            TypeParameters = ImmutableArray.Empty
            ArgumentType = (true, argType) |> ResolvedType.New
            ReturnType = (true, rType) |> ResolvedType.New
            Information = intrinsic
        }
        let varDecl name t (s, e) = {
            VariableName = name |> NonNullable<string>.New |> ValidName
            Type = (true, t) |> ResolvedType.New
            InferredInformation = InferredExpressionInformation.New (false, false)
            Position = Null
            Range = {Line = 1; Column = s}, {Line = 1; Column = e}
        }
        let udtPair range = UserDefinedType (udtName "Microsoft.Quantum" "Pair" range)
        ...
        [
            ...
            |> fun x -> x.ToJson()
            |> asAttribute "SpecializationDeclaration"
...

            {
                ...
                ArgumentTuple   = [ varDecl "p" (Some ((1,27), (1,31)) |> udtPair) (25,26) |> QsTupleItem].ToImmutableArray() |> QsTuple
                Signature       = simpleSignature (None |> udtPair) UnitType
                Documentation   = ImmutableArray.Empty
            } 
            |> fun x -> x.ToJson()
            |> asAttribute "CallableDeclaration"
            {
                ...
                ArgumentTuple   = ImmutableArray.Empty |> QsTuple
                Signature       = simpleSignature UnitType UnitType
                Documentation   = ImmutableArray.Empty
            } 
            |> fun x -> x.ToJson()
            |> asAttribute "CallableDeclaration"
            {
                ...
                Signature       = simpleSignature tupleIntIntType (udtPair None)
            ...
            |> asAttribute "CallableDeclaration"
            {
                ...
                ArgumentTuple   = [varDecl "__Item1__" Int (1,1) |> QsTupleItem; varDecl "__Item2__" Int (1,1) |> QsTupleItem].ToImmutableArray() |> QsTuple
                Signature       = simpleSignature tupleIntIntType (UserDefinedType (udtName "Microsoft.Quantum" "Unused" None))
                Documentation   = ImmutableArray.Create("type constructor for user defined type")
            } 
            |> fun x -> x.ToJson()
            |> asAttribute "CallableDeclaration"
        ...
        |> testHeaders @"Circuits\EmptyElements.qs"
