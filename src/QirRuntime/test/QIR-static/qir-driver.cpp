...
struct QubitCallablesQAPI : public Microsoft::Quantum::CQuantumApiBase
{
    int lastId = -1;
    const int maxQubits;
    ...
    QubitCallablesQAPI(int maxQubits)
        : maxQubits(maxQubits)
...
{
    unique_ptr<QubitCallablesQAPI> qapi = make_unique<QubitCallablesQAPI>(101);
    SetCurrentQuantumApiForQIR(qapi.get());
