...
namespace Microsoft.Quantum.Testing.QIR  {
    ...
    operation AsrtMeasAlloc1OKTest() : Unit {
        ...
        AssertMeasurement(           [PauliZ], [qubit], One,      "Newly allocated qubit after X() must be in the |1> state.");
        AssertMeasurementProbability([PauliZ], [qubit], One, 1.0, "Newly allocated qubit after X() must be in the |1> state.", 1e-10);
    }
    ...
    operation AsrtMeasProbAlloc1HalfProbTest() : Unit {
        ...
        AssertMeasurementProbability([PauliX], [qubit], Zero, 0.5, "Newly allocated qubit must be in the |0> state.", 1e-10);
        AssertMeasurementProbability([PauliX], [qubit],  One, 0.5, "Newly allocated qubit must be in the |0> state.", 1e-10);
        // Measuring in PauliY: 50% |0>, 50% |1>
        AssertMeasurementProbability([PauliY], [qubit], Zero, 0.5, "Newly allocated qubit must be in the |0> state.", 1e-10);
        AssertMeasurementProbability([PauliY], [qubit],  One, 0.5, "Newly allocated qubit must be in the |0> state.", 1e-10);
    }
    ...
    operation AsrtMeasSPlusMinusTest() : Unit {
        ...
        AssertMeasurement(           [PauliY], [qubit], Zero, str);
        // 50% probability in other Pauli bases:
        AssertMeasurementProbability([PauliZ], [qubit], Zero, 0.5, str, 1e-10);
        AssertMeasurementProbability([PauliZ], [qubit],  One, 0.5, str, 1e-10);
        AssertMeasurementProbability([PauliX], [qubit], Zero, 0.5, str, 1e-10);
        AssertMeasurementProbability([PauliX], [qubit],  One, 0.5, str, 1e-10);
        ...
        H(qubit);   // |->
        S(qubit);   // (|0> - i|1>) / SQRT(2))
        AssertMeasurement(           [PauliY], [qubit], One, str);
        // 50% probability in other Pauli bases:
        AssertMeasurementProbability([PauliZ], [qubit], Zero, 0.5, str, 1e-10);
        AssertMeasurementProbability([PauliZ], [qubit],  One, 0.5, str, 1e-10);
        AssertMeasurementProbability([PauliX], [qubit], Zero, 0.5, str, 1e-10);
    ...
    operation AsrtMeas0011() : Unit {
        ...
        AssertMeasurement(           [PauliZ, PauliZ], [left, right], Zero,      str);
        AssertMeasurementProbability([PauliZ, PauliZ], [left, right], Zero, 1.0, str, 1E-05);
        AssertMeasurementProbability([PauliZ, PauliZ], [left, right],  One, 0.0, str, 1E-05);

        X(left);        // |1>
        X(right);       // |1>
        AssertMeasurement(           [PauliZ, PauliZ], [left, right], Zero,      str);    // |11>  (+1 eigenstate)
        AssertMeasurementProbability([PauliZ, PauliZ], [left, right], Zero, 1.0, str, 1E-05);
        AssertMeasurementProbability([PauliZ, PauliZ], [left, right],  One, 0.0, str, 1E-05);

        H(left);        // |->
        H(right);       // |->
        AssertMeasurement(           [PauliX, PauliX], [left, right],  One,      str);
        // 50% probability in other Pauli bases:
        AssertMeasurementProbability([PauliZ, PauliZ], [left, right], Zero, 0.5, str, 1E-05);
        AssertMeasurementProbability([PauliZ, PauliZ], [left, right],  One, 0.5, str, 1E-05);
        AssertMeasurementProbability([PauliY, PauliY], [left, right], Zero, 0.5, str, 1E-05);
        AssertMeasurementProbability([PauliY, PauliY], [left, right],  One, 0.5, str, 1E-05);
    }
    ...
    operation AsrtMeas4qubits() : Unit {
        ...
        AssertMeasurement(           [PauliZ, PauliZ], [qubitIds[0], qubitIds[1]], Zero, "");  // |00> or |11>
        AssertMeasurement(           [PauliZ, PauliZ], [qubitIds[1], qubitIds[2]],  One, "");  // |01> or |10>
        AssertMeasurement(           [PauliZ, PauliZ], [qubitIds[2], qubitIds[3]], Zero, "");  // |00> or |11>
        AssertMeasurementProbability([PauliZ, PauliZ], [qubitIds[0], qubitIds[1]], Zero, 1.0, "", 1E-05);   // |00> or |11>
        AssertMeasurementProbability([PauliZ, PauliZ], [qubitIds[1], qubitIds[2]],  One, 1.0, "", 1E-05);   // |01> or |10>
        AssertMeasurementProbability([PauliZ, PauliZ], [qubitIds[2], qubitIds[3]], Zero, 1.0, "", 1E-05);   // |00> or |11>
    }
