...
TEST_CASE("Qubit Counts", "[QubitManagerBasic]")
{
    ...
    qubitid_t* qubits = new qubitid_t[disabledQubitCount];
    ...
    qubits = new qubitid_t[extraQubitCount];
    ...
    qubits = new qubitid_t[totalQubitCount];
...
TEST_CASE("Allocation of released qubits when reuse is encouraged", "[QubitManagerBasic]")
{
    ...
    qubitid_t q0b = qm->Allocate();
    qubitid_t q1a = qm->Allocate();
...
TEST_CASE("Extending capacity", "[QubitManager]")
{
    ...
    qubitid_t q0 = qm->Allocate();
    REQUIRE(q0 == 0);
    qubitid_t q1 = qm->Allocate(); // This should double capacity
    REQUIRE(q1 == 1);
    REQUIRE(qm->GetFreeQubitCount() == 0);
    REQUIRE(qm->GetAllocatedQubitCount() == 2);
    ...
    qubitid_t q0a = qm->Allocate();
    REQUIRE(q0a == 0);
    qubitid_t q2 = qm->Allocate(); // This should double capacity again
    REQUIRE(q2 == 2);
    REQUIRE(qm->GetFreeQubitCount() == 1);
...
TEST_CASE("Restricted Area", "[QubitManager]")
{
    ...
    qubitid_t q2 = qm->Allocate();
    REQUIRE(q2 == 2);
    qm->Release(q2);

    qubitid_t q2a = qm->Allocate(); // Getting the same one as the one that's just released.
    REQUIRE(q2a == 2);
...
TEST_CASE("Nested Restricted Areas", "[QubitManager]")
{
    ...
    for (int i = 0; i < n; i++)
    {
        qubitid_t q = qm->Allocate(); // Reuse qubit from previous area
    ...
    qubitid_t* qubits = new qubitid_t[n];
