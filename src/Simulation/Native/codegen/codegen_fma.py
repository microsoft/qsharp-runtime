...
################ @@@DBW: Start of _MSC_VER code block ##################
  def kaa(s): kernelarray.append(s)

  kaa("#else\n")
  kaa("    std::intptr_t zero = 0;\n")
  kaa("    std::intptr_t dmask = dsorted[0]")
  for i in range(n-1): kaa(" + dsorted["+str(i+1)+"]")
  kaa(                     ";\n")
  kaa("\n")
  kaa("    if (ctrlmask == 0){\n")
  kaa("        auto thrdFnc= [](intptr_t& dmask, std::intptr_t& zero,V &psi,M const& m")
  if only_one_matrix: kaa(") {\n")
  else:               kaa(",M const& mt) {\n")
  kaa("            return [&](unsigned i) {\n")
  kaa("                if ((i & dmask) == zero)\n")
  kaa("                    kernel_core(psi, i")
  for i in range(n):            kaa(", dsorted[" + str(n-1-i) + "]")
  if only_one_matrix:           kaa(", m);\n")
  else:                         kaa(", m, mt);\n")
  kaa("            }\n")
  kaa("        pl::async_par_for(0,outer,thrdFnc(dmask,zero,psi,m,mt),thrds);\n")

  # if controlmask != 0
  kaa("     } else {\n")
  kaa("        auto thrdFnc= [](intptr_t& ctrlmask,intptr_t& dmask, std::intptr_t& zero,V &psi,M const& m")
  if only_one_matrix: kaa(") {\n")
  else:               kaa(",M const& mt) {\n")
  kaa("            return [&](unsigned i) {\n")
  kaa("                if ((i & ctrlmask) == ctrlmask && (i & dmask) == zero)\n")
  kaa("                    kernel_core(psi, i")
  for i in range(n):            kaa(", dsorted[" + str(n-1-i) + "]")
  if only_one_matrix:           kaa(", m);\n")
  else:                         kaa(", m, mt);\n")
  kaa("            }\n")
  kaa("        pl::async_par_for(0,outer,thrdFnc(ctrlmask,dmask,zero,psi,m,mt),thrds);\n")
  kaa("     }\n")
  kaa("#endif\n")
  kaa("}\n")
