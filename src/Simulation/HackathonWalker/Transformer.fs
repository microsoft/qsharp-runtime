...
// TODO Diagnostics
    ...
    type private ConditionalChangerStatement(stm, ctx : CodegenContext) = 
        ...
        override this.onConditionalStatement (node:QsConditionalStatement) =
            ...
            let functionName = 
                let kind = Identifier.GlobalCallable {Namespace = "Microsoft.Hack" |> NonNullable<string>.New; 
                    Name = "ClasicallyControlled" |> NonNullable<string>.New}
                let exTypeKind = QsTypeKind.Function (QsTypeKind.Qubit |> ResolvedType.New, QsTypeKind.UnitType |> ResolvedType.New)
                AutoGeneratedExpression (QsExpressionKind.Identifier (kind, Null)) exTypeKind false

            let ImmutableQubitWithName name = 
                let kind = (Identifier.LocalVariable (name |> NonNullable<string>.New), Null) |> QsExpressionKind.Identifier
                let exTypeKind = QsTypeKind.Qubit
            ...
            if shouldTransform then
                ...
                let controlQubitExpr = 
                    ...
                    | [ (typedExpr, _) ] -> 
                        ...
                        | EQ (lhs, _) -> 
                            match lhs.Expression with
                            | CallLikeExpression (fn, args) -> Some args
                            | _ -> None
                        | _ -> None
                    | _ -> None
                ...
                let argsExpr = QsExpressionKind.ValueTuple ([(Option.get controlQubitExpr); (Option.get operationExpr); (Option.get operationArgExpr)].ToImmutableArray())
                let argsType = [(Option.get controlQubitExpr).ResolvedType; ( Option.get operationExpr).ResolvedType; (Option.get operationArgExpr).ResolvedType].ToImmutableArray() |> TupleType
                let argsTypedExpr = AutoGeneratedExpression argsExpr argsType false
                let exprKind = CallLikeExpression (functionName, argsTypedExpr)
