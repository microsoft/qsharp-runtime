...
function Generate-Nuspec([string] $target) {
    if (Test-Path $target) { 
    ...
    $nuspec = [xml](Get-Content ($target + ".template"))
...
Generate-Nuspec (Join-Path $PSScriptRoot "Microsoft.Quantum.Simulators.nuspec")
Generate-Nuspec (Join-Path $PSScriptRoot "Microsoft.Quantum.Simulators.Type2.nuspec")