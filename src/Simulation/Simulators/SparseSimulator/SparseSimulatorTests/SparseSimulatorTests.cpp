...
namespace SparseSimulatorTests
{
	TEST_CLASS(SparseSimulatorTests)
	{
	...
	public:
		...
		TEST_METHOD(LabelComparisonTest) {
		...
		TEST_METHOD(AssertTest) {
			...
			Assert::ExpectException<std::exception>([&]() {sim.Assert(basis, qubits, true); sim.update_state(); });
			basis = { Basis::PauliZ, Basis::PauliZ, Basis:: PauliI };
			sim.Assert(basis, qubits, false);
			Assert::ExpectException<std::exception>([&]() {sim.Assert(basis, qubits, true); sim.update_state(); });
			basis = { Basis::PauliX, Basis::PauliI, Basis::PauliI };
			Assert::ExpectException<std::exception>([&]() {sim.Assert(basis, qubits, false); sim.update_state(); });
			Assert::ExpectException<std::exception>([&]() {sim.Assert(basis, qubits, true); sim.update_state(); });
		...
		TEST_METHOD(AssertGHZTest) {
			...
			sim.MCX({ 0 }, 2);
			sim.Assert(basis, qubits, false);
			Assert::ExpectException<std::exception>([&]() {sim.Assert(basis, qubits, true); });
			sim.Z(0);
			sim.Assert(basis, qubits, true);
			Assert::ExpectException<std::exception>([&]() {sim.Assert(basis, qubits, false); });
			sim.S(0);
		...
		TEST_METHOD(TeleportationTest)
		{
			...
			for (double test_angle = 0; test_angle < 1.0; test_angle += 0.34) {
				...
				Assert::AreEqual((float)cos(test_angle / 2.0), (float)teleported_qubit_0.real());
				Assert::AreEqual((float)0.0, (float)teleported_qubit_0.imag());
				Assert::AreEqual((float)sin(test_angle / 2.0), (float)teleported_qubit_1.real());
				Assert::AreEqual((float)0.0, (float)teleported_qubit_1.imag());
			}
		}
