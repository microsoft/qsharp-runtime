...
namespace ska
{
    ...
    namespace detailv3
    {
        ...
        inline int8_t log2(size_t value)
        {
            ...
            // Modification: Cast is necessary because VS decides that when right-shifting signed integers,
            // the sign bit should be copied to the new spaces
            return table[(unsigned long long)((value - (value >> 1)) * 0x07EDD5E59A4E28C2) >> 58];
        }
    ...
    struct fibonacci_hash_policy
    {
        ...
        int8_t next_size_over(size_t& size) const
        {
            auto mid_result = detailv3::next_power_of_two(size);
            size = std::max(size_t(2), detailv3::next_power_of_two(size));
            auto result = detailv3::log2(size);
