...
// This has small modifications from the original bytell_hash_map,
// noted in comments
...
namespace ska
{
    ...
    namespace detailv8
    {
        ...
        struct sherwood_v8_constants
        {
            ...
            //
            // Visual Studio requires static casts on the larger numbers
            static constexpr size_t jump_distances[num_jump_distances]
            {
                ...
                5529475, 12437578, 27986421, 62972253, 141700195, 318819126, static_cast<size_t>(717314626),
                static_cast<size_t>(1614000520), static_cast<size_t>(3631437253), static_cast<size_t>(8170829695),
                static_cast<size_t>(18384318876), static_cast<size_t>(41364501751), static_cast<size_t>(93070021080),
                ...
                static_cast<size_t>(5366895564381), static_cast<size_t>(12075513791265), static_cast<size_t>(27169907873235), static_cast<size_t>(61132301007778),
                static_cast<size_t>(137547673121001), static_cast<size_t>(309482258302503), static_cast<size_t>(696335090510256), static_cast<size_t>(1566753939653640),
                static_cast<size_t>(3525196427195653), static_cast<size_t>(7931691866727775), static_cast<size_t>(17846306747368716),
                static_cast<size_t>(40154190394120111), static_cast<size_t>(90346928493040500), static_cast<size_t>(203280588949935750),
                static_cast<size_t>(457381324898247375), static_cast<size_t>(1029107980662394500), static_cast<size_t>(2315492957028380766),
                static_cast<size_t>(5209859150892887590),
            };
        };
        template<typename T>
        ...
        public:
            ...
            public:
                ...
                // Jumps forward by some number of blocks
                // Added for faster quantum state iteration
                templated_iterator& jump_forward(size_t n = 1){
                    if (index < n*BlockSize){
                        current -= (index - 1)/BlockSize;
            ...
            inline iterator find(const FindKey& key)
            {
                ...
                for (;;)
                {
                    // These are positions within the blocks of headers
                    // see https://youtu.be/M2fKMP47slQ?t=2135
                    ...
                    if (first)
                    {
                        // If the first bit is non-zero: return "end()"
                        // end() is the address at the end of the table
                        // It signifies the data is not found
                        // This occurs here because the metadata indicates that the slot is empty
                        // see here https://youtu.be/M2fKMP47slQ
                        // for the description of how the "magic_for_direct_hit" is a bit
                        // telling us whether or not this is a "real" hash value
                        if ((metadata & Constants::bits_for_direct_hit) != Constants::magic_for_direct_hit)
                            return end();
                        first = false;
                    }
                    // Rreturn the element if the key matches the currently indexed element
                    if (compares_equal(key, block->data[index_in_block]))
                        return { block, index };
                    // Get the jump distance index
                    int8_t to_next_index = metadata & Constants::bits_for_distance;
                    // If there is no more jump distance, we have hit the end of the table
                    if (to_next_index == 0)
                        return end();
                    // Go to the next slot indicated by the jump distance
                    // see https://youtu.be/M2fKMP47slQ
