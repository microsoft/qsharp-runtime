...
namespace Microsoft.Quantum.Simulation.Simulators.NewTracer.GateDecomposition
{
    ...
    public class NewDecomposer : IQuantumProcessor
    {
        protected readonly ITracerDecompositions[] Targets;
        ...
        public NewDecomposer(IEnumerable<ITracerDecompositions> targets, SimulatorBase sim)
        {
        ...
        {
            foreach(ITracerDecompositions target in this.Targets)
            {
                target.Z(qubit);
            }
        }
        ...
        {
            Result res = this.Targets[0].M(qubit);
            for(int i = 1; i < this.Targets.Length; i++)
            {
                this.Targets[i].M(qubit);
            }
            return res;
        }
        ...
        public Result Measure(IQArray<Pauli> bases, IQArray<Qubit> qubits)
        {
            ...
            Result res = this.Targets[0].Measure(bases, qubits);
            for (int i = 1; i < this.Targets.Length; i++)
            {
                this.Targets[i].Measure(bases, qubits);
            }
            return res;
        }
        ...
        protected virtual void ApplyWithOneLessControl(ControlledOp op, IQArray<Qubit> controls, Qubit qubit)
        {
            ...
            IQArray<Qubit> oneLessControl = new QArray<Qubit>(controls.Slice(new QRange(2, controls.Length - 1)).Prepend(helper));
