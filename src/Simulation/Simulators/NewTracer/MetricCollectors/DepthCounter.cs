...
namespace Microsoft.Quantum.Simulation.Simulators.NewTracer.MetricCollectors
{
    public class DepthCounter : QuantumProcessorBase, IMetricCollector, IQuantumProcessor, IQubitTraceSubscriber
    {
        ...
        protected Dictionary<PrimitiveOperationsGroups, double> GateTimes; //is dictionary accessing efficient?

        public DepthCounter(Dictionary<PrimitiveOperationsGroups, double> gateTimes)
        {
            this.CurrentState = new DepthState();
            this.GateTimes = new Dictionary<PrimitiveOperationsGroups, double>(gateTimes) 
                ?? throw new ArgumentNullException(nameof(gateTimes));

            foreach (PrimitiveOperationsGroups operation in Enum.GetValues(typeof(PrimitiveOperationsGroups)))
            {
                if (!GateTimes.ContainsKey(operation))
                {
                    throw new ArgumentException("Not all gate times specified!");
                }
            }
        }
        ...
        public override void ControlledZ(IQArray<Qubit> controls, Qubit qubit)
        {
            ...
            {
                this.RecordQubitUse(controls.Append(qubit), GateTimes[PrimitiveOperationsGroups.CZ]);
            }
            else if (controls.Length == 2)
            {
                this.RecordQubitUse(controls.Append(qubit), GateTimes[PrimitiveOperationsGroups.CCZ]);
            }
            else
            {
        ...
        {
            this.RecordQubitUse(qubit, GateTimes[PrimitiveOperationsGroups.T]);
        }
        ...
        public override void R(Pauli axis, double theta, Qubit qubit)
        {
            ...
            {
                this.RecordQubitUse(qubit, GateTimes[PrimitiveOperationsGroups.RZ]);
            }
            else
            {
                throw new NotImplementedException();
        ...
        {
            this.RecordQubitUse(qubits, GateTimes[PrimitiveOperationsGroups.Measure]);
            return null;
        }

        public override Result M(Qubit qubit)
        {
            this.RecordQubitUse(qubit, GateTimes[PrimitiveOperationsGroups.Measure]);
            return null;
        }
        ...
        public double GetAvailableTime(Qubit qubit)
        {
            ...
            return (double)q.ExtractData(this);
        }
