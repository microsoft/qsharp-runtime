...
If ($Env:TOOLS_OUTDIR -eq $null) { $Env:TOOLS_OUTDIR =  (Join-Path $Env:DROPS_DIR "internal_nugets") }
If (-not (Test-Path -Path $Env:TOOLS_OUTDIR)) { [IO.Directory]::CreateDirectory($Env:TOOLS_OUTDIR) }
